...
module tb_bnn_np_bank;
    ...
    //--------------------------------------------------------------------------
    // No X on outputs when np_valid_out is asserted.
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    // cg_bank: sampled on np_valid_out â€” covers mode, popcount range,
    // activation pattern, and mode Ã— activation cross.
    ...
    task automatic test_t02_unique_lanes();
        ...
        for (int j = 0; j < P_N; j++) begin
            ...
            tv[j] = ACC_W'(FAN_IN / 4);  // low threshold â†’ most activate
        end
        drive_image(xv, wm, tv, 1'b0);
    ...
    initial begin
        $display("=====================================================");
        $display(" tb_bnn_np_bank â€” Level-1 CRV-lite");
        ...
        $display("");
        $display("=====================================================");
        $display(" SCOREBOARD SUMMARY â€” tb_bnn_np_bank");
        $display("=====================================================");
